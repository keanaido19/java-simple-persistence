package de.wethinkco.persistence;
...
/**
 * DbTest is a small command-line tool used to check that we can connect to a
 * SQLite database.
 *
 * By default (without any command-line arguments) it attempts to create a
 * SQLite table in an in-memory database. If it succeeds, we assume that all
 * the working parts we need to use SQLite databases are in place and working.
 *
 * The only command-line argument this app understands is
 *  `-f <filename>`
 *  which tells that application to create the test table in a real
 *  (disk-resident) database named by the given filename. Note that the
 *  application _does not delete_ the named file, but leaves it in the filesystem
